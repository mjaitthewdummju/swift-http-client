...
import HTTPTypes
  ...
  final class BidirectionalStreamingURLSessionDelegate: NSObject, URLSessionTaskDelegate,
              ...
              responseBodyStreamSource.enqueueCallback(callbackToken: callbackToken) { result in
                self.hasSuspendedURLSessionTask.withLockedValue { hasSuspendedURLSessionTask in
                  ...
                  case .failure(let error):
                    debug(
                      "Task delegate: response stream callback, cancelling task, error: \(error)"
                    )
                    dataTask.cancel()
                  }
                }
