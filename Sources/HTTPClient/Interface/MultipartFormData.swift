//===----------------------------------------------------------------------===//
//
// This source file is adapted from the Alamofire open source project
//
//===----------------------------------------------------------------------===//
...
public final class MultipartFormData {
  ...
  enum BoundaryGenerator {
    ...
    static func randomBoundary() -> String {
      ...
      return String(format: "swift-http-client.boundary.%08x%08x", first, second)
    }
  ...
  /// An error thrown when multipart form data encoding fails.
  public struct EncodingError: Error, CustomStringConvertible, LocalizedError {
    ...
    public var description: String {
      ...
      case .bodyPartFileNotReachableWithError(let url, let error):
        return "The URL provided is not reachable: \(url). Error: \(error)"
      case .bodyPartFileIsDirectory(let url):
        return "The URL provided is a directory: \(url)"
      case .bodyPartFileSizeNotAvailable(let url):
        return "Could not fetch the file size for the URL provided: \(url)"
      case .bodyPartFileSizeQueryFailedWithError(let url, let error):
        return "Could not fetch the file size for the URL provided: \(url). Error: \(error)"
      case .bodyPartInputStreamCreationFailed(let url):
        return "Failed to create an InputStream for the URL: \(url)"
  ...
  /// An error representing an unexpected input stream length mismatch.
  struct UnexpectedInputStreamLength: Error, CustomStringConvertible {
    let bytesExpected: UInt64
    let bytesRead: UInt64

    var description: String {
      "Unexpected input stream length. Expected \(bytesExpected) bytes, read \(bytesRead) bytes."
    }
  }
  ...
  public func append(_ fileURL: URL, withName name: String, fileName: String, mimeType: String) {
    ...
    //============================================================
      ...
      do {
        ...
        setBodyPartError(
          withReason: .bodyPartFileNotReachableWithError(atURL: fileURL, error: error)
        )
        return
      }
    #endif
    ...
    do {
      ...
      setBodyPartError(
        withReason: .bodyPartFileSizeQueryFailedWithError(forURL: fileURL, error: error)
      )
      return
    }
  ...
  public consuming func writeEncodedData(to fileURL: URL) throws {
    ...
    if fileManager.fileExists(atPath: fileURL.path) {
      throw EncodingError(reason: .outputStreamFileAlreadyExists(at: fileURL))
    } else if !fileURL.isFileURL {
      throw EncodingError(reason: .outputStreamURLInvalid(url: fileURL))
    }

    guard let outputStream = OutputStream(url: fileURL, append: false) else {
      throw EncodingError(reason: .outputStreamCreationFailed(for: fileURL))
    }
  ...
  private func encodeBodyStream(for bodyPart: BodyPart) throws -> Data {
    ...
    while inputStream.hasBytesAvailable {
      ...
      if let error = inputStream.streamError {
        throw EncodingError(reason: .inputStreamReadFailed(error: error))
      }
    ...
    guard UInt64(encoded.count) == bodyPart.bodyContentLength else {
      let error = UnexpectedInputStreamLength(
  ...
  private func writeBodyStream(for bodyPart: BodyPart, to outputStream: OutputStream) throws {
    ...
    while inputStream.hasBytesAvailable && bytesLeftToRead > 0 {
      ...
      if let streamError = inputStream.streamError {
        throw EncodingError(reason: .inputStreamReadFailed(error: streamError))
      }
  ...
  private func write(_ buffer: inout [UInt8], to outputStream: OutputStream) throws {
    ...
    while bytesToWrite > 0, outputStream.hasSpaceAvailable {
      ...
      if let error = outputStream.streamError {
        throw EncodingError(reason: .outputStreamWriteFailed(error: error))
      }
  ...
  private func contentHeaders(
    ...
    headers.append(HTTPField(name: .contentDisposition, value: disposition))
    if let mimeType {
      headers.append(HTTPField(name: .contentType, value: mimeType))
    }
